...
from transformers import DataCollator, DefaultDataCollator, PreTrainedModel, AdamW, get_linear_schedule_with_warmup
...
class Trainer:
    ...
    def __init__(
            ...
            model: PreTrainedModel,
            args: TrainingArguments,
            patience: int,  # Used for early stopping
            output_best_dir: str,
        ...
        self.output_best_dir = output_best_dir
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        # set best metric for early stopping
        best_eval_loss = float("inf")
        unimproved_cnt = 0
        max_cnt = (self.patience // self.args.logging_steps) + 1
        logger.info("  Max patience of logging steps = %d", max_cnt)
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                        ...
                        if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                            ...
                            if self.args.evaluate_during_training:
                                ...
                                if eval_loss < best_eval_loss:
                                    output_best_dir = os.path.join(
                                        self.output_best_dir, f"eval_loss-{int(eval_loss * 10000) / 10000}"
                                    )
                                    self.save_model(output_best_dir)
                                    best_eval_loss = eval_loss
                                else:
                                    unimproved_cnt += 1

                                # delete the redundant
                                while len(os.listdir(self.output_best_dir)) > self.args.save_total_limit:
                                    file_list = os.listdir(self.output_best_dir)
                                    file_list = sorted(file_list, key=lambda x: os.path.getmtime(os.path.join(self.output_best_dir, x)))
                                    logger.info(f"Deleting older best model {file_list[0]} due to args.save_total_limit")
                                    shutil.rmtree(os.path.join(self.output_best_dir, file_list[0]))
                ...
                if unimproved_cnt >= max_cnt:
                    logger.info("Achieve the patience, trigger early stopping")
                    break

                if 0 < self.args.max_steps < self.global_step or unimproved_cnt >= max_cnt:
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            if not prediction_loss_only:
                ...
                if inputs.get("tgt_token") is not None:
                    label_ids.append(inputs["tgt_token"][:, 1:].detach().cpu().numpy())
        ...
        if metrics["eval_cov_loss"] != 0:
            metrics["eval_loss"] = metrics["eval_tag_loss"] + metrics["eval_gen_loss"]
